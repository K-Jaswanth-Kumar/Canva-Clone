  ...
  try {
    ...
    if (!design) {
      return res.status(404).json({
        success: false,
        message:
          "Design not found! or you don't have permission to access the design",
      });
    }
    ...
    console.error("Error fetching designs by ID", e);
    res.status(500).json({
      success: false,
  ...
  try {
    ...
    if (!design) {
      return res.status(404).json({
        success: false,
        message:
          "Design not found! or you don't have permission to delete the design",
      });
    }
